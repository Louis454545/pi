...
```bash
openshell sandbox create --name morgan-sandbox --from pi -- pi
```
...
```bash
openshell sandbox upload morgan-sandbox ./repo /workspace
openshell sandbox download morgan-sandbox /workspace/repo ./repo-out
```
...
```bash
cp -R packages/morgan-agent/examples/extensions/gondolin ~/.morgan/agent/extensions/gondolin
cd ~/.morgan/agent/extensions/gondolin
...
```bash
cd /path/to/project
pi -e ~/.morgan/agent/extensions/gondolin
```
...
```bash
docker build -t morgan-sandbox -f Dockerfile.morgan .

docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v pi-agent-home:/root/.morgan/agent \
  morgan-sandbox
```

The `-v "$PWD:/workspace"` mounts your current directory into the container at /workspace such that reads and writes in `/workspace` inside Docker directly affect your host files, like in the Gondolin example.

Use a named volume for `/root/.morgan/agent` if you want container-local settings and sessions. Mounting your host `~/.morgan/agent` exposes host auth and session files to the container.
