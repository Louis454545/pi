...
Legacy session files used path-based directories such as `~/.morgan/sessions/--<path>--/<timestamp>_<uuid>.jsonl`. On first global-conversation launch, morgan imports the most recent valid legacy file into the canonical path.

## Archiving Conversations

`/reset` and `/import` archive the previous canonical conversation under `~/.morgan/sessions/global/archive/` before writing a new canonical file.
...
Source on GitHub ([morgan-mono](https://github.com/earendil-works/morgan-mono)):
- [`packages/morgan-agent/src/core/session-manager.ts`](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/morgan-agent/src/core/messages.ts`](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/earendil-works/morgan-mono/blob/main/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/earendil-works/morgan-mono/blob/main/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/@earendil-works/morgan-agent/dist/` and `node_modules/@earendil-works/morgan-ai/dist/`.
...
### Base Message Types (from morgan-ai)
...
### Extended Message Types (from morgan-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if morgan-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name` or `morgan.setSessionName()` in extensions. The `--name` / `-n` flag still parses for compatibility but is deprecated.
