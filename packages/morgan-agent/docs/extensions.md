> morgan can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend morgan's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.morgan/agent/extensions/` (global) or `.morgan/extensions/` (project-local) for auto-discovery. Use `morgan -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `morgan.registerTool()`
- **Proactive triggers** - Start watchers with `morgan.registerTrigger()` that emit structured events for the agent to evaluate
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `morgan.registerCommand()`
- **Session persistence** - Store state that survives restarts via `morgan.appendEntry()`
...
Create `~/.morgan/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.morgan/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.morgan/agent/extensions/*.ts` | Global (all projects) |
| `~/.morgan/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.morgan/extensions/*.ts` | Project-local |
| `.morgan/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as morgan packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@earendil-works/morgan-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@earendil-works/morgan-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@earendil-works/morgan-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed morgan packages installed with `morgan install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (morgan: ExtensionAPI) {
  // Subscribe to events
  morgan.on("event_name", async (event, ctx) => {
  ...
  morgan.registerTool({ ... });
  morgan.registerCommand("name", { ... });
  morgan.registerShortcut("ctrl+x", { ... });
  morgan.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, morgan awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `morgan.registerProvider()` are flushed.
...
export default async function (morgan: ExtensionAPI) {
  ...
  morgan.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `morgan --list-models`.
...
thinking level changes (settings, keybinding, morgan.setThinkingLevel())
...
Fired before pi decides whether to trust a project with dynamic configs (`.morgan` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.
...
```typescript
morgan.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
morgan.on("session_start", async (event, ctx) => {
...
After a successful reset, import, or advanced switch action, morgan emits `session_shutdown` for the old extension instance, reloads and rebinds extensions, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
morgan.on("session_before_fork", async (event, ctx) => {
...
```typescript
morgan.on("session_before_tree", async (event, ctx) => {
...
morgan.on("session_tree", async (event, ctx) => {
...
```typescript
morgan.on("session_shutdown", async (event, ctx) => {
...
```typescript
morgan.on("before_agent_start", async (event, ctx) => {
...
morgan.on("agent_end", async (event, ctx) => {
...
```typescript
morgan.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

morgan.on("turn_end", async (event, ctx) => {
...
```typescript
morgan.on("message_start", async (event, ctx) => {
  // event.message
});

morgan.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

morgan.on("message_end", async (event, ctx) => {
...
```typescript
morgan.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

morgan.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

morgan.on("tool_execution_end", async (event, ctx) => {
...
```typescript
morgan.on("context", async (event, ctx) => {
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Morgan's system prompt string rather than the final serialized provider payload.

```typescript
morgan.on("before_provider_request", (event, ctx) => {
...
```typescript
morgan.on("after_provider_response", (event, ctx) => {
...
```typescript
morgan.on("model_select", async (event, ctx) => {
...
```typescript
morgan.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `morgan.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, morgan waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@earendil-works/morgan-agent";
...
```typescript
import { isBashToolResult } from "@earendil-works/morgan-agent";

morgan.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@earendil-works/morgan-agent";

morgan.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap morgan's built-in local bash backend
...
```typescript
morgan.on("input", async (event, ctx) => {
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while morgan is idle.
...
Request a graceful shutdown of morgan.
...
Returns Morgan's current system prompt string.
...
```typescript
morgan.on("before_agent_start", (event, ctx) => {
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `morgan` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@earendil-works/morgan-agent";

morgan.registerCommand("switch", {
...
- Captured old `morgan` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
morgan.registerCommand("reload-runtime", {
...
export default function (morgan: ExtensionAPI) {
  ...
  morgan.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      morgan.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### morgan.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### morgan.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`morgan.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `morgan.getAllTools()` and are callable by the LLM without `/reload`.

Use `morgan.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@earendil-works/morgan-ai";
...
Placement:
- Any normal extension location works, including configured global extension paths, project-local `.morgan/extensions/` under an explicit working context, morgan packages, or `--extension`.
...
After creating or editing a trigger extension, call `/reload` or the `reload` tool so Morgan starts the new trigger runtime.

### morgan.sendMessage(message, options?)
...
### morgan.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
morgan.sendUserMessage("What is 2+2?");

// With content array (text + images)
morgan.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
morgan.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
morgan.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### morgan.appendEntry(customType, data?)

Persist extension state (does NOT participate in LLM context).

```typescript
morgan.appendEntry("my-state", { count: 42 });
...
### morgan.setSessionName(name)

Set the conversation display name.

```typescript
morgan.setSessionName("Refactor auth module");
```

### morgan.getSessionName()

Get the current session name, if set.

```typescript
const name = morgan.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### morgan.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
morgan.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
morgan.setLabel(entryId, undefined);
...
### morgan.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, morgan keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@earendil-works/morgan-tui";

morgan.registerCommand("deploy", {
...
```typescript
const commands = morgan.getCommands();
...
### morgan.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for messages with your `customType`. See [Custom UI](#custom-ui).

### morgan.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
morgan.registerShortcut("ctrl+shift+p", {
...
### morgan.registerFlag(name, options)

Register a CLI flag.

```typescript
morgan.registerFlag("plan", {
...
// Check value
if (morgan.getFlag("plan")) {
  // Plan mode enabled
}
```

### morgan.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await morgan.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### morgan.getActiveTools() / morgan.getAllTools() / morgan.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools.

```typescript
const active = morgan.getActiveTools();
const all = morgan.getAllTools();
// [{
//   name: "read",
...
morgan.setActiveTools(["read", "bash"]); // Switch to read-only
```

`morgan.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await morgan.setModel(model);
  if (!success) {
...
### morgan.getThinkingLevel() / morgan.setThinkingLevel(level)
...
```typescript
const current = morgan.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh"
morgan.setThinkingLevel("high");
```
...
```typescript
morgan.events.on("my:event", (data) => { ... });
morgan.events.emit("my:event", { ... });
```

### morgan.registerProvider(name, config)

Register or override a model provider dynamically. Useful for proxies, custom endpoints, or team-wide model configurations.

Calls made during the extension factory function are queued and applied once the runner initialises. Calls made after that — for example from a command handler following a user setup flow — take effect immediately without requiring a `/reload`.

If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. morgan waits for the factory before startup continues, so the registered models are available immediately, including to `morgan --list-models`.

```typescript
// Register a new provider with custom models
morgan.registerProvider("my-proxy", {
  name: "My Proxy",
...
morgan.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
morgan.registerProvider("corporate-ai", {
...
### morgan.unregisterProvider(name)
...
```typescript
morgan.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    morgan.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `morgan.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `morgan.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@earendil-works/morgan-agent";
...
import { Type } from "typebox";
import { StringEnum } from "@earendil-works/morgan-ai";
import { Text } from "@earendil-works/morgan-tui";
...
morgan.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via morgan.exec (captured from extension closure)
    const result = await morgan.exec("some-command", [], { signal });
...
**Important:** Use `StringEnum` from `@earendil-works/morgan-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when morgan resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
Extensions can override built-in tools (`read`, `bash`, `edit`, `write`, `reload`, `task_stop`, `monitor`, `subagent`, `grep`, `find`, `ls`) by registering a tool with the same name. Interactive mode displays a warning when this happens.

```bash
# Extension's read tool replaces built-in read
morgan -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
morgan --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@earendil-works/morgan-agent";
...
For `user_bash`, extensions can reuse morgan's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
} from "@earendil-works/morgan-agent";
...
export default function (morgan: ExtensionAPI) {
  ...
  morgan.registerTool({ name: "db_connect", ... });
  morgan.registerTool({ name: "db_query", ... });
  morgan.registerTool({ name: "db_close", ... });

  morgan.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@earendil-works/morgan-agent";
...
// Terminal title
ctx.ui.setTitle("morgan - my-project");
...
```typescript
morgan.on("session_start", (_event, ctx) => {
  ctx.ui.addAutocompleteProvider((current) => ({
    triggerCharacters: ["#"],
...
```typescript
import { Text, Component } from "@earendil-works/morgan-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@earendil-works/morgan-agent";
import { matchesKey } from "@earendil-works/morgan-tui";
...
morgan.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `morgan.sendMessage()`:
...
```typescript
import { highlightCode, getLanguageFromPath } from "@earendil-works/morgan-agent";
...
| `event-bus.ts` | Inter-extension events | `morgan.events` |
| **Session Metadata** |||
