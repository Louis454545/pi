> morgan can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to morgan's agent capabilities. Use it to embed morgan in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @earendil-works/morgan-agent
```
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `morgan.sendMessage()`.
...
The `Agent` class (from `@earendil-works/morgan-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.morgan/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.morgan/extensions/`)
- Project skills (`.morgan/skills/`)
- Project prompts (`.morgan/prompts/`)
- Context files (`AGENTS.md` walking up from cwd)
- Session directory naming

`agentDir` is used by `DefaultResourceLoader` for:
- Global extensions (`extensions/`)
- Global skills (`skills/` under `agentDir`, for example `~/.morgan/agent/skills/`)
...
import { AuthStorage, ModelRegistry } from "@earendil-works/morgan-agent";
...
// Default: uses ~/.morgan/agent/auth.json and ~/.morgan/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
- Built-in tool names: `read`, `bash`, `edit`, `write`, `reload`, `task_stop`, `monitor`, `subagent`, `grep`, `find`, `ls`
- Default built-ins: `read`, `bash`, `edit`, `write`, `reload`, `task_stop`, `monitor`, `subagent`
- `noTools: "all"` disables all tools
- `noTools: "builtin"` disables default built-ins while keeping extension and custom tools enabled
- `excludeTools` disables specific built-in, extension, or custom tool names after any `tools` allowlist is applied

The `edit` tool returns `details.diff` for Morgan's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
import { createAgentSession, defineTool } from "@earendil-works/morgan-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `morgan.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `morgan.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.morgan/agent/extensions/`, `.morgan/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `morgan.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/morgan-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/morgan-agent";
...
1. Global: `~/.morgan/agent/settings.json`
2. Project: `<cwd>/.morgan/settings.json`
...
createMorganTools
createReadOnlyTools
createReadTool, createBashTool, createEditTool, createWriteTool, createReloadTool
createTaskStopTool, createMonitorTool
