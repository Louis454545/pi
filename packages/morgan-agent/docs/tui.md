> morgan can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@earendil-works/morgan-tui`](https://github.com/earendil-works/morgan-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@earendil-works/morgan-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `MORGAN_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@earendil-works/morgan-tui";
...
**In custom tools** via `morgan.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = morgan.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@earendil-works/morgan-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@earendil-works/morgan-tui";
```
...
```typescript
import { matchesKey, Key } from "@earendil-works/morgan-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@earendil-works/morgan-tui";
...
```typescript
import { getMarkdownTheme } from "@earendil-works/morgan-agent";
import { Markdown } from "@earendil-works/morgan-tui";
...
Set `MORGAN_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
MORGAN_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@earendil-works/morgan-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@earendil-works/morgan-agent";
import { DynamicBorder } from "@earendil-works/morgan-agent";
import { Container, type SelectItem, SelectList, Text } from "@earendil-works/morgan-tui";
...
```typescript
import { BorderedLoader } from "@earendil-works/morgan-agent";

morgan.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@earendil-works/morgan-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@earendil-works/morgan-agent";
import { Container, type SettingItem, SettingsList, Text } from "@earendil-works/morgan-tui";

morgan.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while morgan is streaming a response.
...
// Restore morgan's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@earendil-works/morgan-tui";
