...
- `.morgan/settings.json`
- `.morgan/extensions`, `.morgan/skills`, `.morgan/prompts`, or `.morgan/themes`
- `.morgan/SYSTEM.md` or `.morgan/APPEND_SYSTEM.md`
- project `.agents/skills` in the current directory or an ancestor directory

A bare `.morgan` directory does not count as a project resource that requires trust.

When an interactive session starts in a project with resources that require trust and no saved decision for the current directory or a parent directory, pi follows `defaultProjectTrust` from global settings. The default value is `"ask"`, which asks whether to trust the project when UI is available. Saved decisions are stored by canonical directory in `~/.morgan/agent/trust.json`, and the closest saved decision on the current or parent path applies before the global default.

Trusting a project allows pi to load project resources that require trust, including:

- `.morgan/settings.json`
- `.morgan` resources such as extensions, skills, prompt templates, themes, and system prompt files
...
- avoid mounting host `~/.morgan/agent` unless the container should access host sessions, settings, and credentials
...
To report a security issue, follow the repository [Security Policy](https://github.com/earendil-works/morgan-mono/blob/main/SECURITY.md). Do not open a public issue for security-sensitive reports.
