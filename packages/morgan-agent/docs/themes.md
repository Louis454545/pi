> morgan can create themes. Ask it to build one for your setup.
...
On first run, morgan detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.morgan/agent/themes
vim ~/.morgan/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/morgan/main/packages/morgan-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, morgan reloads it automatically for immediate visual feedback.
...
Morgan uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, morgan falls back to the nearest approximation.
