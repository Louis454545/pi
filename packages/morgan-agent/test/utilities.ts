/**
 * Shared test utilities for morgan-agent tests.
 */
...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@earendil-works/morgan-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.morgan/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".morgan", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.morgan/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.morgan/agent/auth.json
 */
...
/** Path to the real morgan agent config directory */
export const MORGAN_AGENT_DIR = join(homedir(), ".morgan", "agent");

/**
 * Get an AuthStorage instance backed by ~/.morgan/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `morgan-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	const agent = new Agent({
		...
		initialState: {
			...
			tools: createMorganTools(process.cwd()),
		},
	});
