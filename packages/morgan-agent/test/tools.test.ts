...
describe("Morgan Agent Tools", () => {
	let testDir: string;

	beforeEach(() => {
		// Create a unique temporary directory for each test
		testDir = join(tmpdir(), `morgan-agent-test-${Date.now()}`);
...
describe("edit tool fuzzy matching", () => {
	...
	beforeEach(() => {
		testDir = join(tmpdir(), `morgan-agent-fuzzy-test-${Date.now()}`);
		mkdirSync(testDir, { recursive: true });
	});
...
describe("edit tool CRLF handling", () => {
	...
	beforeEach(() => {
		testDir = join(tmpdir(), `morgan-agent-crlf-test-${Date.now()}`);
		mkdirSync(testDir, { recursive: true });
	});
