...
import type { AgentEvent } from "@earendil-works/morgan-agent-core";
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = join(tmpdir(), `morgan-rpc-test-${Date.now()}`);
		client = new RpcClient({
			cliPath: join(__dirname, "..", "dist", "cli.js"),
			cwd: join(__dirname, ".."),
			env: { MORGAN_AGENT_DIR: sessionDir },
			provider: "anthropic",
	...
	test("should export to JSONL", async () => {
		await client.start();
		...
		// Export
		const result = await client.exportJsonl();
