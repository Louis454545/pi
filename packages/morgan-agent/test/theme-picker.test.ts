...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "morgan-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("MORGAN_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.MORGAN_AGENT_DIR!, "themes", "foo.json");
