...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.MORGAN_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.MORGAN_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .morgan skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .morgan", async () => {
			const extDir = join(tempDir, ".morgan", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".morgan"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".morgan", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".morgan", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".morgan", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".morgan", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".morgan", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json morgan.extensions in extensions setting", async () => {
			// Create a package with morgan.extensions in package.json
			...
			// Should find the extensions declared in package.json morgan.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .morgan/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-morgan", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-morgan"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-morgan\ndescription: user morgan\n---\n");
		...
		it("should use the project .morgan dir as baseDir for project .morgan skills", async () => {
			const projectBaseDir = join(tempDir, ".morgan");
			const skillPath = join(projectBaseDir, "skills", "project-morgan", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-morgan"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-morgan\ndescription: project morgan\n---\n");
	...
	describe(".agents/skills", () => {
		...
		it("should not auto-discover user ~/.agents/skills", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".morgan", "agent");
		...
		it("should still load explicit skill directories", async () => {
			const explicitSkillsDir = join(tempDir, "external-skills");
			mkdirSync(join(explicitSkillsDir, "nested-skill"), { recursive: true });
			const rootSkill = join(explicitSkillsDir, "root-file.md");
			const nestedSkill = join(explicitSkillsDir, "nested-skill", "SKILL.md");
			writeFileSync(rootSkill, "---\nname: root-file\ndescription: Root markdown file\n---\n");
			writeFileSync(nestedSkill, "---\nname: nested-skill\ndescription: Nested skill\n---\n");
			settingsManager.setSkillPaths([explicitSkillsDir]);
		...
		it("should dedupe user skill entries when ~/.morgan/agent/skills is a symlink", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
			...
			try {
				...
				symlinkSync(externalSkillsDir, agentSkillsDir, directoryLinkType);

				const skillPath = join(externalSkillsDir, "foo", "SKILL.md");
				mkdirSync(join(externalSkillsDir, "foo"), { recursive: true });
				writeFileSync(skillPath, "---\nname: foo\ndescription: foo\n---\n");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .morgan auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".morgan\n");

			const skillDir = join(tempDir, ".morgan", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with morgan manifest", async () => {
		...
		it("should keep morgan manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.morgan\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".morgan", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the morgan-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .morgan settings base", () => {
			...
			const rel = relative(join(tempDir, ".morgan"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in morgan manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json morgan.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".morgan", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".morgan", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".morgan", "npm", "node_modules", "example");
