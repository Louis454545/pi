...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `morgan-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(morgan) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = morgan;
			},
			(morgan) => {
		...
		morgan!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
