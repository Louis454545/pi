...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@earendil-works/morgan-ai";
import { getApiProvider } from "@earendil-works/morgan-ai";
import { getOAuthProvider } from "@earendil-works/morgan-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `morgan-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
