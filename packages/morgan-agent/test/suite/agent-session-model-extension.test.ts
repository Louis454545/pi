import type { AgentTool, ThinkingLevel } from "@earendil-works/morgan-agent-core";
import { fauxAssistantMessage, fauxToolCall, type Model } from "@earendil-works/morgan-ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(morgan) => {
					morgan.on("model_select", async (event) => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(morgan) => {
					morgan.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
