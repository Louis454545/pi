...
// Regression for https://github.com/earendil-works/morgan/issues/5080
//
...
function createSessionManager(options: { sessionFile?: string } = {}): SessionManager {
	return {
		...
		getSessionDir: () => "/tmp/morgan-sessions",
		usesDefaultSessionDir: () => true,
	} as unknown as SessionManager;
}

function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "morgan-shutdown-resume-hint-"));
	tempDirs.push(dir);
