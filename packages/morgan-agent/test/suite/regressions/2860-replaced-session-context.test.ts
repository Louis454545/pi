...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `morgan-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale morgan/ctx", async () => {
		const events: string[] = [];
		...
		const { runtime } = await createRuntimeForTest(
			(morgan) => {
				...
				morgan.on("session_start", () => {
					events.push(`start:${currentInstance}`);
				});
				morgan.on("session_shutdown", () => {
					events.push(`shutdown:${currentInstance}`);
				});
				morgan.registerCommand("repro", {
					description: "repro",
	...
	it("supports withSession for fork", async () => {
		const { runtime } = await createRuntimeForTest(
			(morgan) => {
				morgan.registerCommand("fork-it", {
					description: "fork-it",
					handler: async (_args, ctx) => {
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(morgan) => {
				morgan.registerCommand("switch-it", {
