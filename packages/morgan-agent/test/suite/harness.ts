/**
 * Local test harness for the new morgan-agent test suite.
 */

import { existsSync, mkdirSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import type { AgentMessage, AgentTool } from "@earendil-works/morgan-agent-core";
import { Agent } from "@earendil-works/morgan-agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@earendil-works/morgan-ai";
import { registerFauxProvider } from "@earendil-works/morgan-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `morgan-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
export async function createHarness(options: HarnessOptions = {}): Promise<Harness> {
	...
	const sessionManager = options.persistSession
		? SessionManager.openGlobal(agentDir, { cwd: tempDir, sessionDir: join(tempDir, "sessions") })
