...
import { createAssistantMessageEventStream } from "@earendil-works/morgan-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `morgan-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`);
