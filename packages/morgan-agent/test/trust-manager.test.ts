...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".morgan", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".morgan", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".morgan", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".morgan"), { recursive: true });
			writeFileSync(join(cwd, ".morgan", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".morgan"), { recursive: true, force: true });
