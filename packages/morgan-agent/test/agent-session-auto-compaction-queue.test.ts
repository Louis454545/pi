...
describe("AgentSession auto-compaction queue resume", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `morgan-auto-compaction-queue-${Date.now()}`);
	...
	it("should ignore stale pre-compaction assistant usage on pre-prompt compaction checks", async () => {
		...
		sessionManager.appendCompaction("summary", firstKeptEntryId, staleAssistant.usage.totalTokens, undefined);
	...
	it("should not trigger threshold compaction for error messages when only kept pre-compaction usage exists", async () => {
		...
		const firstKeptEntryId = sessionManager.getEntries()[0]!.id;
		sessionManager.appendCompaction("summary", firstKeptEntryId, keptAssistant.usage.totalTokens, undefined);
