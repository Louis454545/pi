...
function parseArgs(argv: string[]): Args {
	...
	let sessionPath = resolve(join(tmpdir(), `morgan-sdk-codex-cache-probe-tool-loop-${Date.now()}.jsonl`));
...
Notes:
  - Uses createAgentSession() from the morgan-agent SDK
...
function buildPrompt(turn: number): string {
	...
	const lines: string[] = [];
	lines.push("This is an automated prompt-cache tool-loop test executed through the morgan morgan-agent SDK.");
...
async function main(): Promise<void> {
	...
	const resourceLoader = createMinimalResourceLoader(
		"You are participating in a prompt-cache benchmark through the morgan-agent SDK. This is a real test. Follow each user instruction exactly. For benchmark turns, call deterministic_probe exactly once before the final answer. Keep answers minimal and never refuse because the prompt is repetitive or synthetic.",
	);
