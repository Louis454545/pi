...
import { type AutocompleteProvider, CombinedAutocompleteProvider, Container } from "@earendil-works/morgan-tui";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.morgan/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.morgan/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.morgan/extensions",
				}),
			},
			{
				path: "/tmp/project/.morgan/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.morgan/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.morgan/extensions",
			...
			{
				path: "/tmp/project/.morgan/npm/node_modules/morgan-markdown-preview/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.morgan/npm/node_modules/morgan-markdown-preview/extensions/index.ts",
					{
						source: "npm:morgan-markdown-preview",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.morgan/npm/node_modules/morgan-markdown-preview",
					},
				),
			},
			{
				path: "/tmp/project/.morgan/npm/node_modules/@scope/morgan-scoped/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.morgan/npm/node_modules/@scope/morgan-scoped/extensions/index.ts",
					{
						source: "npm:@scope/morgan-scoped",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.morgan/npm/node_modules/@scope/morgan-scoped",
					},
				),
			},
			{
				path: "/tmp/project/.morgan/git/github.com/HazAT/morgan-interactive-subagents/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.morgan/git/github.com/HazAT/morgan-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/morgan-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.morgan/git/github.com/HazAT/morgan-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.morgan/git/github.com/HazAT/morgan-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.morgan/git/github.com/HazAT/morgan-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/morgan-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  @scope/morgan-scoped, answer.ts, cli-extension.ts, HazAT/morgan-interactive-subagents, HazAT/morgan-interactive-subagents:subagents, local-index, morgan-markdown-preview, user-index"`);
	});
...
"[Extensions]
  project
    /tmp/project/.morgan/extensions/answer.ts
    /tmp/project/.morgan/extensions/local-index
    git:github.com/HazAT/morgan-interactive-subagents
      extensions
      extensions/subagents
    npm:@scope/morgan-scoped
      extensions
    npm:morgan-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "morgan-mono");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".morgan", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.morgan/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.morgan/agent/AGENTS.md");
		expect(output).toContain("~/Development/morgan-mono/AGENTS.md");
		expect(output).not.toContain("~/.morgan/agent/AGENTS.md, AGENTS.md");
	});
