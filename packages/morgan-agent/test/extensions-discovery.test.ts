...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "morgan-ext-test-"));
	...
	const extensionCode = `
		export default function(morgan) {
			morgan.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json morgan field", async () => {
	...
	it("keeps package.json morgan extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with morgan field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without morgan field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ morgan: { extensions: ["./entry.ts"] } }));
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(morgan) {
				morgan.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(morgan) {
				morgan.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(morgan) {
				morgan.on("agent_start", async () => {});
				morgan.on("tool_call", async (event) => undefined);
				morgan.on("agent_end", async () => {});
			}
		`;
	...
	it("loads extension with shortcuts", async () => {
		const extCode = `
			export default function(morgan) {
				morgan.registerShortcut("ctrl+t", {
