...
import { Agent, type AgentEvent, type AgentTool } from "@earendil-works/morgan-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@earendil-works/morgan-ai";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `morgan-retry-test-${Date.now()}`);
