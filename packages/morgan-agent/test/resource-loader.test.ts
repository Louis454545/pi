...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "morgan-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".morgan", "prompts");
			...
			const projectSkillDir = join(cwd, ".morgan", "skills", "collision-skill");
			mkdirSync(userSkillDir, { recursive: true });
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".morgan", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".morgan", "themes"), { recursive: true });
	...
	morgan.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".morgan"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".morgan", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".morgan", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".morgan", "extensions");
...
export default function(pi) {
	...
	pi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".morgan", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	morgan.registerCommand("project-only", {
	...
	morgan.registerCommand("user-only", {
...
Content`,
		...
		it("should skip project resources when project resources are disabled", async () => {
			...
			mkdirSync(join(morganDir, "prompts"), { recursive: true });
			mkdirSync(join(morganDir, "skills", "project-skill"), { recursive: true });
			writeFileSync(join(cwd, "AGENTS.md"), "# Project Guidelines\n\nBe helpful.");
			writeFileSync(join(morganDir, "SYSTEM.md"), "Project system prompt");
			writeFileSync(join(morganDir, "APPEND_SYSTEM.md"), "Project append prompt");
			writeFileSync(join(morganDir, "prompts", "project.md"), "Project prompt");
			writeFileSync(
				join(morganDir, "skills", "project-skill", "SKILL.md"),
		...
		it("should discover SYSTEM.md from trusted cwd/.morgan", async () => {
			const morganDir = join(cwd, ".morgan");
...
Project skill content`,
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const morganDir = join(cwd, ".morgan");
			mkdirSync(morganDir, { recursive: true });
			writeFileSync(join(morganDir, "APPEND_SYSTEM.md"), "Additional instructions.");
