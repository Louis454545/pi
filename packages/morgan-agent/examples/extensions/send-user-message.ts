 ...
 *
 * Demonstrates morgan.sendUserMessage() for sending user messages from extensions.
 * Unlike morgan.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (morgan: ExtensionAPI) {
	...
	morgan.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			morgan.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	morgan.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				morgan.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	morgan.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				morgan.sendUserMessage(args, { deliverAs: "followUp" });
	...
	morgan.registerCommand("askwith", {
