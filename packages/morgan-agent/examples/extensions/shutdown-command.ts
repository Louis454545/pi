 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit morgan cleanly.
 */
...
export default function (morgan: ExtensionAPI) {
	// Register a /quit command that cleanly exits morgan
	morgan.registerCommand("quit", {
		description: "Exit morgan cleanly",
	...
	morgan.registerTool({
		...
		description: "Complete a task and exit morgan",
	...
	morgan.registerTool({
		...
		description: "Deploy the application and exit morgan",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await morgan.exec("npm", ["run", "deploy", params.environment], { signal });
