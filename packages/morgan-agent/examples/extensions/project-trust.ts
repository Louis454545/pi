 ...
 *
 *   mkdir -p ~/.morgan/agent/extensions
 *   cp packages/morgan-agent/examples/extensions/project-trust.ts ~/.morgan/agent/extensions/
 *
 * Or:
 *
 *   morgan -e packages/morgan-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .morgan, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@earendil-works/morgan-agent";
...
export default function (morgan: ExtensionAPI) {
	...
	morgan.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
