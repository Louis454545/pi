...
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@earendil-works/morgan-tui";
...
export default function (morgan: ExtensionAPI) {
	morgan.registerCommand("invaders", {
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom((tui, _theme, _kb, done) => {
				return new SpaceInvadersComponent(
					...
					() => done(undefined),
					(state) => {
						morgan.appendEntry(INVADERS_SAVE_TYPE, state);
					},
					savedState,
				);
			});
