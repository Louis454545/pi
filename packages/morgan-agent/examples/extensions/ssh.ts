 ...
 *
 * Usage:
 *   morgan -e ./ssh.ts --ssh user@host
 *   morgan -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (morgan: ExtensionAPI) {
	morgan.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	morgan.on("session_start", async (_event, ctx) => {
		...
		const arg = morgan.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
