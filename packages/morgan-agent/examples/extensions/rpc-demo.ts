...
export default function (morgan: ExtensionAPI) {
	...
	morgan.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "morgan RPC Demo (new session)" : "morgan RPC Demo");
	...
	morgan.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	morgan.on("turn_end", async (_event, ctx) => {
	...
	morgan.registerCommand("rpc-input", {
	...
	morgan.registerCommand("rpc-editor", {
	...
	morgan.registerCommand("rpc-prefill", {
