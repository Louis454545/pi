 ...
 *
 * Runs Morgan's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 *
 * Setup:
 *   cd packages/morgan-agent/examples/extensions/gondolin
 *   npm install --ignore-scripts
 *
 * Usage:
 *   cd /path/to/project
 *   morgan -e /path/to/morgan/packages/morgan-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (morgan: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `morgan ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	morgan.registerCommand("gondolin", {
	...
	morgan.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
