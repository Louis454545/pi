 ...
 *
 * Usage: morgan --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@earendil-works/morgan-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/morgan-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/morgan-tui";
...
export default function (morgan: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like morgan-doom would need)
	morgan.registerCommand("overlay-animation", {
	...
	morgan.registerCommand("overlay-anchors", {
	...
	morgan.registerCommand("overlay-margins", {
	...
	morgan.registerCommand("overlay-stack", {
	...
	morgan.registerCommand("overlay-overflow", {
	...
	morgan.registerCommand("overlay-edge", {
	...
	morgan.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	morgan.registerCommand("overlay-maxheight", {
	...
	morgan.registerCommand("overlay-sidepanel", {
	...
	morgan.registerCommand("overlay-toggle", {
	...
	morgan.registerCommand("overlay-passive", {
	...
	morgan.registerCommand("overlay-focus", {
	...
	morgan.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/morgan-mono/packages/morgan-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/morgan-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like morgan-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (morgan-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
