...
export default function (morgan: ExtensionAPI) {
	morgan.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await morgan.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[morgan] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await morgan.exec("git", ["add", "-A"]);
		const { code: commitCode } = await morgan.exec("git", ["commit", "-m", commitMessage]);
