...
import type { Component, EditorTheme, TUI } from "@earendil-works/morgan-tui";
import { truncateToWidth, visibleWidth } from "@earendil-works/morgan-tui";
...
export default function (morgan: ExtensionAPI) {
	...
	morgan.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await morgan.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = morgan.getThinkingLevel();
