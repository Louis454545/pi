...
async function checkDirtyRepo(
	...
	const { stdout, code } = await morgan.exec("git", ["status", "--porcelain"]);
...
export default function (morgan: ExtensionAPI) {
	morgan.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(morgan, ctx, action);
	});

	morgan.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(morgan, ctx, "fork");
	});
}
