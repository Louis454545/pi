 ...
 * (logo + keybinding hints) with a custom component showing the morgan mascot.
 */

import type { ExtensionAPI, Theme } from "@earendil-works/morgan-agent";
import { VERSION } from "@earendil-works/morgan-agent";

// --- MORGAN MASCOT ---
// Based on the Morgan agent character.
function getMorganMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const morganBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${morganBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${morganBlue(BLOCK.repeat(2))}    ${morganBlue(BLOCK.repeat(2))}`;
...
export default function (morgan: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getMorganMascot(theme);
						// Add a subtitle with hint
						const subtitle = `${theme.fg("muted", "   shitty universal agent")}${theme.fg("dim", ` v${VERSION}`)}`;
	...
	morgan.registerCommand("builtin-header", {
