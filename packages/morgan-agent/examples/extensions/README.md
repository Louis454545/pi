...
Example extensions for morgan-agent.

## Usage

```bash
# Load an extension with --extension flag
morgan --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.morgan/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `morgan.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs morgan theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `morgan.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using morgan-ai's built-in Anthropic/OpenAI streaming via proxy |
