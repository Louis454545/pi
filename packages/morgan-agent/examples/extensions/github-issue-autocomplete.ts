...
async function resolveGitHubRepo(morgan: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await morgan.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (morgan: ExtensionAPI): void {
	morgan.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(morgan, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await morgan.exec(
					"gh",
					[
						"issue",
						"list",
