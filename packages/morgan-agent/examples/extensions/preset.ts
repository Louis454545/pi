 ...
 * - ~/.morgan/agent/presets.json (global)
 * - <cwd>/.morgan/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `morgan --preset plan` - start with plan preset
...
import { DynamicBorder, getAgentDir } from "@earendil-works/morgan-agent";
import type { Api, Model } from "@earendil-works/morgan-ai";
import { Container, Key, type SelectItem, SelectList, Text } from "@earendil-works/morgan-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".morgan", "presets.json");
...
export default function presetExtension(morgan: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: morgan.getThinkingLevel(),
				tools: morgan.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			morgan.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = morgan.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				morgan.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.morgan/agent/presets.json or .morgan/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await morgan.setModel(originalState.model);
				}
				morgan.setThinkingLevel(originalState.thinkingLevel);
				morgan.setActiveTools(originalState.tools);
			} else {
				morgan.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	morgan.registerShortcut(Key.ctrlShift("u"), {
	...
	morgan.on("before_agent_start", async (event) => {
	...
	morgan.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = morgan.getFlag("preset");
	...
	morgan.on("turn_start", async () => {
		if (activePresetName) {
			morgan.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
