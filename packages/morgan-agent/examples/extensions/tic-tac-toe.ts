...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@earendil-works/morgan-agent";
import { StringEnum } from "@earendil-works/morgan-ai";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@earendil-works/morgan-tui";
...
export default function (morgan: ExtensionAPI) {
	morgan.on("session_start", async (_event, ctx) => reconstructState(ctx));
	morgan.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	morgan.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	morgan.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	morgan.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			morgan.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						morgan.appendEntry(SAVE_TYPE, getBoardDetails());
