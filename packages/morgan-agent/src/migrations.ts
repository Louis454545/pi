...
import { CONFIG_DIR_NAME, getAgentDir, getBinDir, getMorganHomeDir } from "./config.ts";
import { migrateKeybindingsConfig } from "./core/keybindings.ts";
import { isLegacyEnvVarNameConfigValue } from "./core/resolve-config-value.ts";
import { stripJsonComments } from "./utils/json.ts";

const MIGRATION_GUIDE_URL =
	"https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/CHANGELOG.md#extensions-migration";
const EXTENSIONS_DOC_URL =
	"https://github.com/earendil-works/morgan-mono/blob/main/packages/morgan-agent/docs/extensions.md";
...
/**
 * Migrate sessions from ~/.morgan/agent/*.jsonl to proper session directories.
 *
 * Bug in v0.30.0: Sessions were saved to ~/.morgan/agent/ instead of
 * ~/.morgan/sessions/<encoded-cwd>/. This migration moves them
 * to the correct location based on the cwd in their session header.
 *
 * See: https://github.com/earendil-works/morgan-mono/issues/320
 */
export function migrateSessionsFromAgentRoot(): void {
	...
	for (const file of files) {
		try {
			...
			const correctDir = join(morganHomeDir, "sessions", safePath);
 ...
 * Note: tools/ may contain fd/rg binaries extracted by morgan, so only warn if it has other files.
 */
