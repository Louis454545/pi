...
import * as _bundledMorganAgentCore from "@earendil-works/morgan-agent-core";
import * as _bundledMorganAi from "@earendil-works/morgan-ai";
import * as _bundledMorganAiOauth from "@earendil-works/morgan-ai/oauth";
import type { KeyId } from "@earendil-works/morgan-tui";
import * as _bundledMorganTui from "@earendil-works/morgan-tui";
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@earendil-works/morgan-agent-core": _bundledMorganAgentCore,
	"@earendil-works/morgan-tui": _bundledMorganTui,
...
function getAliases(): Record<string, string> {
	...
	const morganAgentEntry = packageIndex;
	const morganAgentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@earendil-works/morgan-agent-core");
	const morganTuiEntry = resolveWorkspaceOrImport("tui/dist/index.js", "@earendil-works/morgan-tui");
	const morganAiEntry = resolveWorkspaceOrImport("ai/dist/index.js", "@earendil-works/morgan-ai");
	const morganAiOauthEntry = resolveWorkspaceOrImport("ai/dist/oauth.js", "@earendil-works/morgan-ai/oauth");
	...
	_aliases = {
		...
		"@earendil-works/morgan-ai/oauth": morganAiOauthEntry,
...
function readMorganManifest(packageJsonPath: string): MorganManifest | null {
	try {
		const content = fs.readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content);
		if (pkg.morgan && typeof pkg.morgan === "object") {
			return pkg.morgan as MorganManifest;
		}
		return null;
	} catch {
		return null;
 ...
 *
 * Checks for:
 * 1. package.json with "morgan.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "morgan" field first
	const packageJsonPath = path.join(dir, "package.json");
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readMorganManifest(packageJsonPath);
