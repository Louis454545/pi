...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@earendil-works/morgan-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@earendil-works/morgan-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown morgan and exit. Available in all contexts. */
	shutdown(): void;
...
export interface TriggerEventInput {
	/** Stable event id for deduplication. If omitted, Morgan generates one. */
	eventId?: string;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Morgan loaded without re-discovering resources. */
...
/** Configuration for registering a provider via morgan.registerProvider(). */
...
export type ExtensionFactory = (morgan: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for morgan.* API methods.
