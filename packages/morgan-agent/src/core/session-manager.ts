...
import { getAgentDir as getDefaultAgentDir, getMorganHomeDir, getSessionsDir } from "../config.ts";
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, undefined/false if morgan-generated (backward compatible) */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, false if morgan-generated */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.morgan/sessions/.
 */
...
function getDefaultSessionDirPath(cwd: string, agentDir: string = getDefaultAgentDir()): string {
	const resolvedMorganHomeDir = resolvePath(getMorganHomeDir(agentDir));
	return join(resolvedMorganHomeDir, "sessions", getSafeSessionPath(cwd));
}
...
export function migrateLegacyDefaultSessionRoot(agentDir: string = getDefaultAgentDir()): void {
	const sessionsDir = join(resolvePath(getMorganHomeDir(agentDir)), "sessions");
...
export async function acquireGlobalConversationLock(
	...
	try {
		...
		if (code === "ELOCKED") {
			throw new Error(
				"Global conversation is already open in another morgan process. Close it before starting another persistent session.",
			);
		}
		throw error;
	}
...
export class SessionManager {
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.morgan/sessions/<encoded-cwd>/).
	 */
	 ...
	 * @param sessionDir Optional session directory for branch sessions. If omitted, derives from file's parent.
