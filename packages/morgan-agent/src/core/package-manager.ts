...
import { getBundledBrowserHarnessDir, getBundledSkillsDir } from "../config.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.MORGAN_OFFLINE;
 ...
 *
 * Explicit global entries precede auto-discovered global entries, packages,
 * and bundled defaults.
 */
function resourcePrecedenceRank(m: PathMetadata): number {
	if (m.source === "bundled") return 5;
	if (m.origin === "package") return 4;
	return 2 + (m.source === "local" ? 0 : 1);
}
...
type SkillDiscoveryMode = "morgan" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "morgan" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readMorganManifestFile(packageJsonPath: string): MorganManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { morgan?: MorganManifest };
		return pkg.morgan ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readMorganManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "morgan");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	async resolve(onMissing?: (source: string) => Promise<MissingSourceAction>): Promise<ResolvedPaths> {
		...
		this.addBundledResources(accumulator, globalSettings);
	 ...
	 * Used to detect duplicate package sources.
	...
	/**
	 * Dedupe packages by package identity.
	 */
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside morgan and resolve morgan APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @earendil-works/morgan-* peers. Stale auto-installed morgan peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "morgan-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = this.readMorganManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof MorganManifest];
	...
	private readMorganManifest(packageRoot: string): MorganManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		const addResources = (resourceType: ResourceType, paths: string[]): void => {
			const target = this.getTargetMap(accumulator, resourceType);
			for (const path of paths) {
				this.addResource(
					...
					path,
					metadata,
					isEnabledByOverrides(path, overrides[resourceType], globalBaseDir),
				);
			}
		};
		addResources("extensions", collectAutoExtensionEntries(directories.extensions));
		addResources("skills", collectAutoSkillEntries(directories.skills, "morgan"));
		addResources("prompts", collectAutoPromptEntries(directories.prompts));
		addResources("themes", collectAutoThemeEntries(directories.themes));
	}
	...
	private addBundledResources(
		...
		const overrides = (globalSettings.skills ?? []) as string[];
		const metadata: PathMetadata = {
			source: "bundled",
			scope: "temporary",
			origin: "top-level",
