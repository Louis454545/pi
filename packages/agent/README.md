# @earendil-works/morgan-agent-core

Stateful agent with tool execution and event streaming. Built on `@earendil-works/morgan-ai`.

## Installation

```bash
npm install @earendil-works/morgan-agent-core
```
...
```typescript
declare module "@earendil-works/morgan-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/morgan-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/morgan-agent-core";
