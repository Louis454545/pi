...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".morgan/skills")),
		source("user", join(homedir(), ".morgan/agent/skills")),
		source("path", join(env.cwd, "../../../morgan-skills")),
	],
	(skill, source) => ({ ...skill, source }),
);
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".morgan/prompts")), source("user", join(homedir(), ".morgan/agent/prompts"))],
