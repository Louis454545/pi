...
export function createReloadToolDefinition(
	...
	return {
		...
		label: "reload",
		description:
			"Schedule a pi runtime reload after modifying files that affect the running agent, including extensions, skills, prompts, themes, settings, or keybindings.",
		...
		promptGuidelines: [
			...
			"Agent-affecting files include pi extensions, skills, prompt templates, themes, keybindings, settings, AGENTS.md/project instructions, and other files loaded into the system prompt or command/autocomplete runtime.",
		...
		async execute() {
			...
			return {
				content: [
					{
						type: "text",
						text: "Reload scheduled. Pi will reload runtime resources before the next model turn.",
					},
				],
