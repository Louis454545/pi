...
function ensureMemoryHome(piHomeDir: string, now: Date): string {
	...
	const dailyMemoryPath = join(piHomeDir, "memories", "daily", `${day}.md`);
	ensureFile(
		dailyMemoryPath,
...
function readIdentityFiles(piHomeDir: string): MemoryFileRead[] {
	return IDENTITY_FILE_NAMES.map((name) => {
		const path = join(piHomeDir, name);
...
function formatIdentitySection(files: MemoryFileRead[], piHomeDir: string, dailyMemoryPath: string): string {
	const lines = [
		"# Agent Identity Context",
		"",
		`Pi global home: ${piHomeDir}`,
...
function findDailyMemoryCandidates(piHomeDir: string, query: string | undefined): DailyMemoryCandidate[] {
	const tokens = tokenizeQuery(query);
	if (tokens.size === 0) {
		return [];
	}

	const dailyDir = join(piHomeDir, "memories", "daily");
...
export function loadAgentMemoryPromptContext(
	...
	const piHomeDir = getPiHomeDir(options.agentDir);
	const dailyMemoryPath = ensureMemoryHome(piHomeDir, now);
	const identityFiles = readIdentityFiles(piHomeDir);
	const candidates = findDailyMemoryCandidates(piHomeDir, options.query);

	return {
		piHomeDir,
		dailyMemoryPath,
		identitySection: formatIdentitySection(identityFiles, piHomeDir, dailyMemoryPath),
