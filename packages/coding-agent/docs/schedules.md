...
Schedules are trusted project-local TypeScript modules that run while pi is open. They live under `.pi/schedules` in the active working context and are loaded only when the working context is explicit, such as `pi --cwd <path>` or after `/cwd <path>`.
...
- Schedules run only while pi is open.
- Cron schedules do not catch up missed runs after pi was closed.
- A schedule never overlaps with itself. If it becomes due while already running, pi records one pending run and starts it after the current run finishes.
...
After writing or editing `.pi/schedules` files, ask the agent to call:
