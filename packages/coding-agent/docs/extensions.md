...
Extensions can override built-in tools (`read`, `bash`, `edit`, `write`, `reload`, `task_stop`, `monitor`, `grep`, `find`, `ls`) by registering a tool with the same name. Interactive mode displays a warning when this happens.
