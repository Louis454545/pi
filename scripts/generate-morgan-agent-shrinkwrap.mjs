...
const morganAgentDir = join(repoRoot, "packages/morgan-agent");
const rootLockfilePath = join(repoRoot, "package-lock.json");
const shrinkwrapPath = join(morganAgentDir, "npm-shrinkwrap.json");
const internalPackagePrefix = "@earendil-works/morgan-";
...
function generateShrinkwrap() {
	...
	const morganAgentPackage = readJson(join(morganAgentDir, "package.json"));
	const internalWorkspaces = getInternalWorkspaces(lockPackages);
	const shrinkwrapPackages = {
		"": copyPackageJsonEntry(morganAgentPackage, { includeName: true }),
	};
	const addedPaths = new Set([""]);
	const internalNames = new Set();
	const queue = Object.keys(packageDependencies(morganAgentPackage)).map((name) => ({ name, from: "" }));
	...
	const shrinkwrap = {
		name: morganAgentPackage.name,
		version: morganAgentPackage.version,
		lockfileVersion: 3,
		requires: true,
...
try {
	...
	if (checkOnly) {
		if (!existsSync(shrinkwrapPath)) {
			console.error("packages/morgan-agent/npm-shrinkwrap.json is missing.");
			console.error("Run: npm run shrinkwrap:morgan-agent");
			process.exit(1);
		}
		const current = readFileSync(shrinkwrapPath, "utf8");
		if (current !== content) {
			console.error("packages/morgan-agent/npm-shrinkwrap.json is out of date.");
			console.error("Run: npm run shrinkwrap:morgan-agent");
			process.exit(1);
		}
		console.log("packages/morgan-agent/npm-shrinkwrap.json is up to date.");
	} else {
		writeFileSync(shrinkwrapPath, content);
		const packageCount = Object.keys(shrinkwrap.packages).length - 1;
		const platformPackageCount = Object.values(shrinkwrap.packages).filter((entry) => entry.os || entry.cpu || entry.libc).length;
		console.log(
			`Wrote packages/morgan-agent/npm-shrinkwrap.json (${packageCount} packages, ${platformPackageCount} platform-specific).`,
		);
	}
} catch (error) {
