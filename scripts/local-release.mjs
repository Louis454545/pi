...
const packages = [
	{ directory: "packages/ai", name: "@earendil-works/morgan-ai" },
	{ directory: "packages/tui", name: "@earendil-works/morgan-tui" },
	{ directory: "packages/agent", name: "@earendil-works/morgan-agent-core" },
	{ directory: "packages/morgan-agent", name: "@earendil-works/morgan-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "morgan-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `morgan-${platform}.zip` : `morgan-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createMorganShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "morgan.cmd"))) {
			writeFileSync(join(installDirectory, "morgan.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\morgan.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "morgan.ps1"), '& "$PSScriptRoot/node_modules/.bin/morgan.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "morgan.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\morgan.exe" %*\r\n');
		writeFileSync(join(installDirectory, "morgan.ps1"), '& "$PSScriptRoot/node_modules/.bin/morgan.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "morgan"), join(installDirectory, "morgan"));
}
...
if (rootPackageJson.name !== "morgan-monorepo") {
...
if (!options.skipInstall) {
	...
	createMorganShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createMorganShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `morgan-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "morgan.exe" : "morgan")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "morgan.cmd" : "morgan")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "morgan.cmd" : "morgan")} --help`);
	}
}
