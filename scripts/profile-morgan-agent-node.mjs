...
const agentDirEnvName = "MORGAN_AGENT_DIR";
const startupBenchmarkEnvName = "MORGAN_STARTUP_BENCHMARK";

function printHelp() {
	console.log(`Usage:
  node scripts/profile-morgan-agent-node.mjs [options]

Profiles morgan-agent startup with the runtime selected below:
- npm run profile:tui     -> builds packages/morgan-agent and profiles TUI startup with Node
- npm run profile:rpc     -> builds packages/morgan-agent and profiles RPC startup with Node
...
Options:
  ...
  --agent-dir <dir>      Use a specific MORGAN_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force MORGAN_OFFLINE=1 / MORGAN_SKIP_VERSION_CHECK=1
...
async function runBuild() {
	process.stdout.write("Building packages/tui, packages/ai, packages/agent, and packages/morgan-agent...\n");
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.MORGAN_OFFLINE = "1";
		env.MORGAN_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "morgan-startup-benchmark-")) : undefined;
...
async function main() {
	...
	if (runtime === "bun") {
		process.stdout.write(
			`Using Bun runtime with ${options.mode === "rpc" ? "packages/morgan-agent/src/cli.ts --mode rpc" : "packages/morgan-agent/src/cli.ts"}\n`,
		);
	}
