...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@earendil-works/morgan-agent";
import { Container, Text } from "@earendil-works/morgan-tui";
...
async function fetchAdvisoryMetadata(morgan: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await morgan.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(morgan, cwd, target);
	}
	...
	try {
		const result = await morgan.exec("gh", args);
...
export default function promptUrlWidgetExtension(morgan: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = morgan.getSessionName()?.trim();
		if (!currentName) {
			morgan.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(morgan, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	morgan.on("session_switch", async (_event, ctx) => {
