...
If you need stronger boundaries, containerize or sandbox Pi. See [packages/morgan-agent/docs/containerization.md](packages/morgan-agent/docs/containerization.md) for three patterns:

- **OpenShell**: run the whole `morgan` process in a policy-controlled sandbox.
- **Gondolin extension**: keep `morgan` and provider auth on the host while routing built-in tools and `!` commands into a local Linux micro-VM.
- **Plain Docker**: run the whole `morgan` process in a local container for simple isolation.
